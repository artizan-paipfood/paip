...
enum QtyFlavorsPizza {
  ...
  static QtyFlavorsPizza fromInt(int qty) => QtyFlavorsPizza.values.firstWhere((e) => e.qty == qty);
}
...
// List<ComplementModel> complements;
  ...
  double get getPrice => (promotionalPrice ?? 0) > 0 ? promotionalPrice! : price;

  List<ComplementModel> get getComplementsSortIndex =>
      complements.where((complement) => complement.visible).sorted((a, b) => a.index.compareTo(b.index));
  List<ComplementModel> get getComplementsSortAZ => complements.where((element) => element.visible).sorted((a, b) => a.name.compareTo(b.name));

  String get imagePath => "${establishmentId}_$id.png";
  String get imagePathThumb => "thumb-${establishmentId}_$id.png";
  String get imageCacheIdThumb => "thumb-$imageCacheId";
  ...
  bool get quickAdd => complements.isEmpty && sizes.isEmpty;

  CartProductDto buildCartProductDto() {
    final SizeModel? size = sizes.firstWhereOrNull((element) => element.isPreSelected);
    return CartProductDto(id: uuid, product: this, itemsCartMap: {}, qtyFlavorsPizza: qtyFlavorsPizza, price: price + (size?.price ?? 0), size: size);
  }
}
