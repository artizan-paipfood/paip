enum HoursEnum {
  ...
  DateTime buildDateByWeekDay(int weekDay) {
    final now = DateTime.now();
    if (now.weekday == 7 && weekDay != now.weekday) {
      return buildNow.add(const Duration(days: 1));
    }
    final int difference = weekDay - buildNow.weekday;
    return buildNow.add(Duration(days: difference));
  }
