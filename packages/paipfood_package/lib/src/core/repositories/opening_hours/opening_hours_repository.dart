...
class OpeningHoursRepository implements IOpeningHoursRepository {
  ...
  @override
  Future<List<OpeningHoursModel>> getByEstablishmentId(String establishmentId) async {
  ...
  @override
  Future<List<OpeningHoursModel>> getByEstablishmentIdAndWeekDAy({required String id, required int weekDay}) async {
    final request = await http.get("/rest/v1/$table?establishment_id=eq.$id&week_day_id=eq.$weekDay&select=*");
    final List list = request.data;
    return list.map<OpeningHoursModel>((openingHours) {
      return OpeningHoursModel.fromMap(openingHours);
    }).toList();
