...
// Map<String, Map<String, ItemCart>>? itemsCartMap
  ...
  factory CartProductDto.fromMap(Map map) {
    ...
    return CartProductDto(
      ...
      qtyFlavorsPizza: map['qty_flavors_pizza'] != null ? QtyFlavorsPizza.fromInt(map['qty_flavors_pizza']) : null,
    );
  }

  String toJson() => json.encode(toMap());

  factory CartProductDto.fromJson(String source) => CartProductDto.fromMap(json.decode(source));
  ...
  bool addItem({required ItemModel item, required ComplementModel complement}) {
    ...
    final bool allowEntries = complementAllowEntries(complement);
    //! Não remover esse comentário
  ...
  bool complementIsValid(ComplementModel complement) {
    if (complement.complementType == ComplementType.pizza) {
      if (getQtyItemsByComplement(complement) >= qtyFlavorsPizza!.qty) return true;
      return false;
    }
    if (complement.qtyMin < 1) return true;
    if (getQtyItemsByComplement(complement) >= complement.qtyMin) return true;
    return false;
  }
  ...
  CartProductDto? agroup(List<CartProductDto> cart) {
    if (observation.isNotEmpty) return null;

    return null;
  }

  // complemto bate e opcao
  // complemento bate em qtd
}
